...
class Pinaka_Shifts_Api_Controller {
	...
	// 	// $log_context2 = array( 'source' => 'pinaka-pos-shift-data' );
	...
	// 	// --- Validate and calculate drawer total ---
	// 	$calculated_drawer_total = 0;
	// 	foreach ($data['drawer_denominations'] as $denom) {
	// 		// echo $denom['denom'] . ' x ' . $denom['denom_count'] .' = ' . $denom['denom'] * $denom['denom_count'];
	// 		// echo '||';
	...
	// 	// --- Validate and calculate tube total ---
	// 	$calculated_tube_total = 0;
	// 	foreach ($data['tube_denominations'] as $item) {
	// 		if (!isset($item['denomination'], $item['tube_count'], $item['cell_count'], $item['total'])) {
	// 			// return new WP_Error('invalid_data', 'Invalid tube denomination format.', ['status' => 400]);
	...
	// 		$calculated_tube_total += $calculated;
	// 	}
	// 	$calculated_tube_total = floatval( $data['tube_total_amount'] );
	// 	// $logger2->info( 'Logger → denominations: ' . print_r( $data['tube_denominations'], true ), $log_context2 );
	// 	// $logger2->info( 'Logger → calculated_tube_total: ' . $calculated_tube_total, $log_context2 );
	// 	$combined_total = floatval( $data['drawer_total_amount'] ) + floatval( $data['tube_total_amount'] );
	// 	if ( abs( $combined_total - floatval( $data['total_amount'] ) ) > 0.01 ) {
	...
	// 		update_post_meta( $post_id, '_shift_drawer_closing_total', floatval( $data['drawer_total_amount'] ) );
	// 		update_post_meta( $post_id, '_shift_tube_closing_denominations', wp_json_encode( $data['tube_denominations'] ) );
	// 		update_post_meta( $post_id, '_shift_tube_closing_total', floatval( $data['tube_total_amount'] ) );
	// 				// --- Set end time ---
	...
	// 		add_post_meta($post_id, '_shift_prev_opening_balance', $combined_total);
	// 		update_post_meta($post_id, '_shift_drawer_denominations', wp_json_encode($data['drawer_denominations']));
	// 		update_post_meta($post_id, '_shift_drawer_total', floatval($data['drawer_total_amount']));
	// 		update_post_meta($post_id, '_shift_tube_denominations', wp_json_encode($data['tube_denominations']));
	// 		update_post_meta($post_id, '_shift_tube_total', floatval($data['tube_total_amount']));
	// 	}
	// 	$prev_open_balances = get_post_meta($post_id, '_shift_prev_opening_balance', false);
	// 	$prev_count = count($prev_open_balances);
	// 	if($prev_count > 0 && $combined_total > 0)
	// 	{
	// 		if($prev_count > 1)
	// 		{
	// 			$opening_balance = $prev_open_balances[$prev_count - 2];
	// 		}
	// 		else
	// 		{
	// 			$opening_balance = $prev_open_balances[$prev_count - 1];
	// 		}
	// 		update_post_meta($post_id, '_shift_opening_balance', $opening_balance);
	// 	}
	// 	// --- Safe drop (optional) ---
	...
	// 	$total_payouts   = $this->get_vendor_cash_payouts( $post_id );
	// 	// $expected_cash = $opening_balance + $total_sales + $calculated_tube_total - $total_payouts - $safe_drop_total;
